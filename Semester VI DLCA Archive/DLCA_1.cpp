...
array<array<float, 3>, 500> points;
array<int, 500> dsu_par;              // Parent array for DSU, note that the parent will also be the color of a DSU tree
array<int, 500> dsu_size;             // Sizes of DSU trees
array<vector<int>, 500> dsu_children; // Children under a leader, used for cluster movement, contains only adjacent children
vector<int> cluster_leader_indices;
array<vector<int>, 500> neighbour;
array<array<float, 3>, 500> previous_verlet_points;
...
int main(int argc, char *argv[])
{
    ...
    for (n_points = 1; n_points <= 500; n_points++)
    {
        n_clusters = n_points;
        ...
        for(float cur_dist=0;cur_dist<=5.0;cur_dist+=0.1)
        {
            int n=0;
            for(int i=0;i<n_points;i++)
