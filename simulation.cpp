...
void initiate_reaction_with_CHx_if_possible(pair<int, int> site)
{
    ...
    if (lattice[site.first][site.second] == "C")
    {
        ...
        if (bonded_H_atoms[site.first][site.second].size() == 4)
        {
            ...
            hydrocarbon_sizes_seen.insert(1);
...
// Finds the terminal nodes for possible chains
void find_leaf_nodes(pair<int, int> site, int incoming_valency, set<pair<int, int>> &vis, vector<pair<int, int>> &leaf_nodes)
{
    if (lattice[site.first][site.second] != "C" || incoming_valency == 4 || incoming_valency + bonded_H_atoms[site.first][site.second].size() > 4)
        return;

    vis.insert(site);

    if (incoming_valency + bonded_H_atoms[site.first][site.second].size() == 4)
        leaf_nodes.push_back(site);
    else
    {
        for (auto neighbour_site : nearest_neighbours[site.first][site.second])
        {
            if(vis.count(neighbour_site))
                continue;
            // cerr<<neighbour_site.first<<','<<neighbour_site.second<<' '<<4 - incoming_valency - bonded_H_atoms[site.first][site.second].size()<<'\n';
            find_leaf_nodes(neighbour_site, 4 - incoming_valency - bonded_H_atoms[site.first][site.second].size(), vis, leaf_nodes);
        }
    }
    vis.erase(site);
}
...
// Search chains
bool search_chain(pair<int, int> site, int incoming_valency, set<pair<int, int>> &vis, vector<pair<int, int>> &chain)
{
    if (lattice[site.first][site.second] != "C" || incoming_valency == 4 || incoming_valency + bonded_H_atoms[site.first][site.second].size() > 4)
        return false;
    ...
    else
    {
        ...
        for (auto neighbour_site : shuffled_neighbours)
        {
            if(vis.count(neighbour_site))
                continue;
            // cerr<<neighbour_site.first<<','<<neighbour_site.second<<' '<<4 - incoming_valency - bonded_H_atoms[site.first][site.second].size()<<'\n';
            if(search_chain(neighbour_site, 4 - incoming_valency - bonded_H_atoms[site.first][site.second].size(), vis, chain))
                return true;
        }
    }
    chain.pop_back();
...
{
    vector<pair<int, int>> leaf_nodes_with_duplicates;
    set<pair<int, int>> vis_nodes;
    
    for(int incoming_valency = 0; incoming_valency < bonded_H_atoms[site.first][site.second].size(); incoming_valency++)
        find_leaf_nodes(site, incoming_valency, vis_nodes, leaf_nodes_with_duplicates);

    set<pair<int, int>> leaf_nodes_set(leaf_nodes_with_duplicates.begin(), leaf_nodes_with_duplicates.end());
    vector<pair<int, int>> leaf_nodes(leaf_nodes_set.begin(), leaf_nodes_set.end());
    shuffle(leaf_nodes.begin(), leaf_nodes.end(), generator);

    for(auto leaf_site: leaf_nodes)
    {
        vector<pair<int, int>> chain;
        set<pair<int, int>> vis;

        if(!search_chain(leaf_site, 0, vis, chain))
            continue;

        hydrocarbon_production++;
        hydrocarbon_sizes_seen.insert(chain.size());
        for (auto C_site : chain)
        {
